...
| 8. Conexão Google Drive no Web & Configuração de Client ID | COMPLETED | Suporte a `google_sign_in: 7.2.0` no Web via `authorizationClient.authorizeScopes`, reatividade MobX com observables dedicados e 279/279 testes aprovados |
| 9. Simplificação do Fluxo Google Drive para Usuário Final | COMPLETED | Fluxo OAuth2 direto sem exigência de configuração manual de Google Cloud pelo usuário leigo; mensagens de erro/cancelamento amigáveis e 278/278 testes aprovados |
